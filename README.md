...
It is probably not necessary to provide a detailed explanation of the connect four game, as most people should be familiar with it. The game consists of a grid of 7 columns and 6 rows which is vertically suspended. Two players take alternating turns in dropping a colored disc into one of the columns. As soon as one of the players has four discs that form a vertical, horizontal, or diagonal line, this player wins the game. If no space is left in any of the columns, the game results in a draw. 
...
* The transition *(s<sub>t</sub>, a<sub>t</sub>, *r<sub>t+1</sub>*, *s<sub>t+1</sub>*)* along with the information if *s<sub>t+1</sub>* is a terminal game state will be stored in a *replay memory* consisting of the last *M* transitions.
...
* From the replay memory, a *mini-batch* of *m* transitions is randomly assembled to perform a backpropagation pass for updating the weights of the neural network to improve the estimate of the Q-function. The inputs *x* to the network are given by the states *s<sub>t</sub>* in the state transitions. The expected output *y* (the *ground truth* in supervised learning) for each sample in the mini-batch is either given directly by the immediate reward *r<sub>t+1</sub>* in case *s<sub>t+1</sub>* is a terminal state, otherwise by computing the Q-learning update rule given above, i.e., performing a forward pass of *s<sub>t+1</sub>* through the network to obtain its estimates of *max<sub>a'</sub> (Q(s<sub>t+1</sub>,a'))* and setting *y = r<sub>t+1</sub> + N * max<sub>a'</sub> (Q(s<sub>t+1</sub>,a'))*, where *N* is the discount factor. The update is only performed for the output of the action *a<sub>t</sub>* in the transition (as only for this action, the transition contains the resulting state *s<sub>t+1</sub>*), so the other values in the expected output vector *y* (i.e., the outputs of the Q-function for all other actions *a* with *a != a<sub>t</sub>*) are set to the network's predictions for the input vector *x*. This only generates gradients (und thus an update to the weights during gradient descent) for the action *a<sub>t</sub>* stored in the transition. For details, please refer to the explanations provided by Mnih et al. in the corresponding paper listed above or directly to the code.
