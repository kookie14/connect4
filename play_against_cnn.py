...
def print_current_board(board):
    ...
    for row in [5, 4, 3, 2, 1, 0]:
        for column in range(0, 7):
            pygame.draw.rect(screen, BLACK ,(column * CELL_SIZE, (row + 1) * CELL_SIZE, CELL_SIZE, CELL_SIZE))
            pygame.draw.circle(screen, WHITE, (column * CELL_SIZE + CELL_SIZE // 2, (row + 1) * CELL_SIZE + CELL_SIZE // 2), CELL_SIZE // 2 - 5)
            
            cell_content = playing_board[column, row]
            if cell_content == -1:
                pygame.draw.circle(screen, RED, (column * CELL_SIZE + CELL_SIZE // 2, (row + 1) * CELL_SIZE + CELL_SIZE // 2), CELL_SIZE // 2 - 5)
            elif cell_content == 1:
                pygame.draw.circle(screen, YELLOW, (column * CELL_SIZE + CELL_SIZE // 2, (row + 1) * CELL_SIZE + CELL_SIZE // 2), CELL_SIZE // 2 - 5)
...
while (not terminal_state[0]):
    ...
    if player == 1:
        ...
        estimates = test_session.run(network_output, {input_x : board_to_one_channel_map(playing_board.get_board())} )[0]
