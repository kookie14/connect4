...
epsilon_start = 1.0
epsilon_end = 0.1
num_epsilon_interpolation_iterations = 20000
...
input_x = tf.placeholder(dtype = tf.float32, shape = [None, 7*6], name = "input_x")
...
num_inputs = 7*6
num_hidden_units_0 = 4096
num_hidden_units_1 = 2048
num_hidden_units_2 = 1024
num_hidden_units_3 = 512
#num_hidden_units_4 = 128
#num_hidden_units_5 = 64
num_outputs = 7

# weights and biases for fully connected layers
weights_0 = tf.Variable(tf.random_normal((num_inputs,num_hidden_units_0), stddev=math.sqrt(2.0/float(num_inputs + num_hidden_units_0))), dtype = tf.float32) 
...
# forward pass
hidden_layer_0_output = tf.nn.relu(tf.add(tf.matmul(input_x, weights_0), bias_0))
...
while episode <= num_games and (not abort_training):
    ...
    while not playing_board.check_terminal_state()[0]:
        ...
        in_vec = board_to_column_vector(current_state)
        ...
        if playing_board.check_column(action):
            ...
            if terminal_state[0]:
                ...
                # opponent should mainly exploit
                tmp_action, _, _ = epsilon_greedy(board_to_column_vector(tmp_board.get_board()), epsilon * 0.1)
                ...
                while not tmp_board.check_column(tmp_action):
                    tmp_action, _, _ = epsilon_greedy(board_to_column_vector(tmp_board.get_board()), epsilon)
        ...
        samples_x = np.zeros([batch_size, 7*6], dtype = np.float32)
        ...
        sample_resulting_states = np.zeros([batch_size, 7*6], dtype = np.float32)
        ...
        for i in range(0,batch_size):
            ...
            samples_x[i,:] = current_sample[0].flatten()
            sample_actions[i] = current_sample[1]
            sample_rewards[i] = current_sample[2]
            sample_resulting_states[i,:] = current_sample[3].flatten()
